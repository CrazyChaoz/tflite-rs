...
{
    fn drop(&mut self) {
        let handle = Box::into_raw(mem::replace(&mut self.handle, Box::new([0u64; 67])));
        #[allow(forgetting_copy_types, clippy::useless_transmute, deprecated)]
        unsafe {
    ...
    Op: OpResolver,
{
    ...
    pub fn allocate_tensors(&mut self) -> Result<()> {
        ...
        #[allow(forgetting_copy_types, deprecated)]
        let r = unsafe {
    ...
    pub fn set_num_threads(&mut self, threads: c_int) {
        ...
        #[allow(forgetting_copy_types, deprecated, clippy::transmute_num_to_bytes)]
        unsafe {
